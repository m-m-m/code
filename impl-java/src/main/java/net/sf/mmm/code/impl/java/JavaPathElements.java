 ...
 * @since 1.0.0
 */
public class JavaPathElements extends JavaNodeItemContainerFlat<CodePathElement, JavaPathElement>
    implements CodePathElements, CodeNodeItemWithGenericParent<JavaPackage, JavaPathElements> {
  ...
  JavaFile getFile(String simpleName, boolean withoutSuperLayer, boolean init) {
    ...
    JavaPathElement child = get(simpleName, withoutSuperLayer);
  ...
  private JavaPackage createPackage(String simpleName, boolean add) {
    ...
    JavaPackage file = new JavaPackage(this.parent, simpleName, null);
    if (add && isMutable()) {
      add(file);
    }
    return file;
  ...
  public JavaType getType(String simpleName, boolean withoutSuperLayer) {
    ...
    for (JavaPathElement child : getList()) {
      if (child.isFile()) {
        JavaType type = ((JavaFile) child).getType(simpleName);
        if (type != null) {
          return type;
        }
      }
