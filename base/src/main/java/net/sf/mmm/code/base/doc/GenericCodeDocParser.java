...
/**
 * Generic parser to parse and apply {@link CodeDoc} read as plain lines of {@link String}s from the source
 * code to {@link net.sf.mmm.code.api.element.CodeElement}s.
 *
 * @author Joerg Hohwiller (hohwille at users.sourceforge.net)
 * @since 1.0.0
 */
public class GenericCodeDocParser {

  private static final Logger LOG = LoggerFactory.getLogger(GenericCodeDocParser.class);
  ...
  private final Map<String, GenericCodeDocTag> argMap;

  private final Map<String, GenericCodeDocTag> genericMap;

  private final Map<String, GenericCodeDocTag> exceptionMap;

  private GenericCodeDocTag returns;

  /**
   * The constructor.
   */
  public GenericCodeDocParser() {
  ...
  private GenericCodeDocTag parseThrowsTag(String line, int tagLength) {
  ...
  private GenericCodeDocTag parseTag(String line, int start, int end, int length, Map<String, GenericCodeDocTag> map, String duplicationLog) {
    ...
    GenericCodeDocTag tag = new GenericCodeDocTag(tagDoc);
    GenericCodeDocTag existing = map.put(name, tag);
    if (existing == null) {
  ...
  private GenericCodeDocTag parseParamTag(String line) {
  ...
  private void parseReturnTag(String line) {
    ...
    if (this.returns == null) {
      this.returns = new GenericCodeDocTag(returnDoc);
    } else {
  ...
  public void parseDoc(CodeOperation operation, List<String> javaDocLines) {
    ...
    for (CodeParameter arg : operation.getParameters()) {
      GenericCodeDocTag argDoc = getArgumentDoc(arg.getName());
      if (argDoc != null) {
        argDoc.put(arg.getDoc());
      }
    }
    for (CodeException exception : operation.getExceptions()) {
      GenericCodeDocTag exceptionDoc = getExceptionDoc(exception.getType());
  ...
  private void applyTypeVariablesDoc(CodeElementWithTypeVariables element) {
    ...
    for (CodeTypeVariable typeParam : element.getTypeParameters()) {
      GenericCodeDocTag typeParamDoc = getTypeParameterDoc(typeParam.getName());
  ...
  private GenericCodeDocTag getTypeParameterDoc(String name) {

    return this.genericMap.get(name);
  }

  private GenericCodeDocTag getExceptionDoc(CodeGenericType exception) {

    GenericCodeDocTag exceptionTag = this.exceptionMap.get(exception.getSimpleName());
