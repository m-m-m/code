...
public abstract interface CodeMutableItem extends CodeItem {
  ...
  /**
   * By default a {@link CodeNode} retrieved from an existing source (e.g. via {@link CodeContext#getType(String)}) is
   * immutable. Use an according {@code edit()} method to get a new mutable copy of the object before you do any
   * changes. A newly created {@link CodeItem} will however always be mutable. In case a {@link CodeItem} is immutable,
   * all setter methods will throw a {@link net.sf.mmm.util.exception.api.ReadOnlyException} and all
