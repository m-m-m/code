...
public abstract class AbstractCodeLanguage implements CodeLanguage {
  ...
  protected String verifyName(CodeItem item, Pattern pattern, String name) {
    ...
    if (name != null) {
      if (!isTypeInDefaultPackage(item) && isRevervedKeyword(name, item)) {
        throw new ObjectMismatchException(name, "no reserved keyword", item.getClass().getSimpleName(), "name");
      }
      if (pattern != null) {
    ...
    if (!valid) {
      throw new ObjectMismatchException(name, pattern, item.getClass().getSimpleName(), "name");
    }
    return name;
  }
  ...
  /**
   * @return the {@link Pattern} that defines valid {@link CodePackage} {@link CodePackage#getSimpleName()
   *         names}.
   */
  protected abstract Pattern getSimpleNamePatternForPackage();

  /**
   * @return the {@link Pattern} that defines valid {@link CodeType#getSimpleName() names} for
   *         {@link CodeType} and {@link CodeFile}.
   */
  protected abstract Pattern getSimpleNamePatternForType();

  /**
   * @param item the item to {@link #verifyName(CodeItemWithName, String) verify}. May be ignored if pattern
   *        and validation is independent of the item type.
   ...
   * @param item the {@link CodeItem} to verify that defines the given {@code name}. Will most probably be
   *        ignored but a language may allow keywords in specific places and forbid in others.
   * @return {@code true} if the given {@code name} is invalid (for the given {@link CodeItem}) because it is
   *         a reserved keyword.
   */
