...
public abstract class JavaSourceCodeReaderLowlevel extends CharReaderScanner {
    ...
    static final CharFilter CHAR_FILTER_NUMBER_LITERAL_START =
        c -> ((c >= '0') && (c <= '9') || (c == '+') || (c == '-'));
    ...
    /**
     * @return the plain JavaDoc lines that have been parsed by the last invocation of {@link #consume()}.
     *         Will be {@link List#isEmpty() empty} for no JavaDoc.
     */
        ...
        while (!expect(')')) {
            ...
            if (key.isEmpty()) {
                if (first) {
                    ...
                    LOG.warn("Annotation {} parameter without name in {}.", annotationTypeName,
                        Character.toString(forcePeek()), file);
                }
            } else {
                parseWhitespacesAndComments();
                if (!expect('=')) {
            ...
            if (value == null) {
                if (expect('{')) {
                    ...
                    if (!expect('}')) {
                        LOG.warn(
                            "Invalid annotation array value - missing closing curly brace '}' for annotation {} at value {}",
        ...
        while (true) {
            ...
            if (CHAR_FILTER_OPERATOR.accept(c)) {
                ...
                if (operator == null) {
                    ...
                    if (operator.isNAry()) {
                        expression =
                            new JavaNAryOperatorExpression((CodeNAryOperator) operator, new ArrayList<>(expressions));
    ...
    /**
     * @param inInterface
     *            - {@code true} if in the context of an interface (where public is the default),
